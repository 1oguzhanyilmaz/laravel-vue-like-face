    ...
    <div class="bg-white rounded shadow w-2/3 mt-6 overflow-hidden">
        ...
        <div class="flex flex-col p-4">
            <div class="flex items-center">
                <!-- ### profile image ### -->
                <div class="w-8">
                    <img :src="post.data.attributes.posted_by.data.attributes.profile_image.data.attributes.path" alt="" class="w-8 h-8 object-cover rounded-full">
                </div>
                <!-- ### profile Name ### -->
                <div class="ml-6">
        ...
        <div class="flex justify-between border-1 border-gray-400 m-4">
            <button class="flex justify-center py-2 rounded-lg text-sm w-full focus:outline-none"
                    :class="[post.data.attributes.likes.user_likes_count ? 'bg-blue-600 text-white' : '']"
        ...
        <div v-if="comments" class="border-t border-gray-400 p-4 pt-2">
            <div class="flex">
                <input type="text" v-model="commentBody" name="comment" class="w-full pl-4 h-8 bg-gray-300 rounded-lg focus:outline-none" placeholder="Your Comment">
                <button v-if="commentBody"
                        class="bg-gray-200 ml-2 px-2 py-1 rounded-lg focus:outline-none hover:bg-blue-500 hover:text-white"
            ...
            <div class="flex my-4 items-center" v-for="comment in post.data.attributes.comments.data">
                <div class="w-8">
                    <img :src="comment.data.attributes.commented_by.data.attributes.profile_image.data.attributes.path" alt="" class="w-8 h-8 object-cover rounded-full">
                </div>
