<template>
    <div class="bg-white rounded shadow w-2/3 p-4">
        <div class="flex justify-between items-center">
            ...
            <div>
                <div class="w-8">
                    <img :src="authUser.data.attributes.profile_image.data.attributes.path" alt="" class="w-8 h-8 object-cover rounded-full">
                </div>
            </div>

            <!-- ### input ### -->
            <div class="flex-1 flex mx-4">
                <input v-model="postMessage" type="text" name="body" class="w-full pl-4 h-8 bg-gray-200 rounded-full focus:outline-none focus:shadow-outline font-sm" placeholder="Add a Post">
    ...
    export default {
        ...
        computed: {
            ...
            postMessage: {
                ...
                // run after Wait 500 milliseconds
                set: _.debounce(function(postMessage){
                    this.$store.commit('updateMessage', postMessage)
                }, 300),
                // set(postMessage){
                //     return this.$store.commit('updateMessage', postMessage);
                // },
            },
            settings(){
                const vm = this;
